    ...
    def recognition(self, fig_path, size):
            ...
            for i in pg.event.get():
                if i.type == pg.MOUSEBUTTONDOWN:
                    ...
                    if len(vertex) == 4:
                        ...
                        pg.display.flip()
        abs_x = max([abs(vertex[i][0] - vertex[i + 1][0]) for i in range(0, 3)])
        abs_y = max([abs(vertex[i][1] - vertex[i + 1][1]) for i in range(0, 3)])
        left_up_pos = vertex[np.argmin([vertex[i][0] + vertex[i][1] for i in range(4)])]
        ...
        for y in range(size):  # row
            ...
            for x in range(size):  # column
                pos_1 = [left_up_pos[0] + (x / size) * abs_x + (abs_x / size) / 2,
                         left_up_pos[1] + (y / size) * abs_y + (abs_y / size) / 2]
