...
class Ant:
    ...
    def __init__(self, start, end, max_step, phe_imp, dis_imp) -> None:
        self.position = None
        self.max_step = max_step  # 蚂蚁最大行动力
        self.phe_imp = phe_imp  # 信息素重要性系数 Importance coefficient of pheromone
    ...
    def run(self, map_data, phe_data):
        self.position = copy.deepcopy(self.start)
        # Step 1:不断找下一个节点，直到走到终点或者力竭
        for i in range(self.max_step):
            r = self.select_next_node(map_data, phe_data)
            if not r:
    ...
    def select_next_node(self, map_data, phe_data):
        """
        Function:
        ---------
        选择下一节点，结果直接存入self.position，仅返回一个状态码True/False标志选择的成功与否。
        """
        y_1 = self.position[0]
        x_1 = self.position[1]
        # Step 1:计算理论上的周围节点
        ...
        for i in range(len(node_be_selected_1)):
            p = (dis_1[i] ** self.dis_imp) * (phe_data[y_1 * len(map_data) + x_1][
...
class ACO(baseAlgorithm):
    def __init__(self, map_data, start=[0, 0], end=[19, 19], max_iter=100, ant_num=50, phe_imp=1, dis_imp=10,
                 evaporate=0.7, phe_init=8) -> None:
        """
            Params:
            --------
                phe_imp : 信息素重要性系数\n
        ...
        """
        # Step 0: 参数定义及赋值
        self.success_way_list = None
        self.max_iter = max_iter  # 最大迭代次数
        self.ant_num = ant_num  # 蚂蚁数量
        self.ant_gener_phe = 1  # 每只蚂蚁携带的最大信息素总量
        self.phe_init = phe_init  # 初始信息素浓度
        ...
        self.map_length = self.map_data.shape[0]  # 地图尺寸,用来标定蚂蚁的最大体力
        self.phe_data = phe_init * np.ones(shape=[self.map_length * self.map_length,
                                                  self.map_length * self.map_length])  # 信息素矩阵
    ...
    def run(self, show_process=False):
        ...
        for i in range(self.max_iter):
            ...
            for j in range(self.ant_num):
                ant = Ant(start=self.ant_params['start'], end=self.ant_params['end'], max_step=self.map_length * 3,
                          phe_imp=self.ant_params['phe_imp'], dis_imp=self.ant_params['dis_imp'])
                ant.run(map_data=self.map_data.copy(), phe_data=self.phe_data)
                if ant.successful:  # 若成功，则记录路径信息
                    self.success_way_list.append(ant.record_way)
            # Step 2:计算每条路径对应的长度，后用于信息素的生成量
            way_length_list = []
            for j in self.success_way_list:
                way_length_list.append(calc_total_length(j))
            # Step 3:更新信息素浓度
            #  step 3.1: 挥发
            self.phe_data = self.evaporate * self.phe_data
            #  step 3.2: 叠加新增信息素
            for k, j in enumerate(self.success_way_list):
                j_2 = np.array(j)
                j_3 = j_2[:, 0] * self.map_length + j_2[:, 1]
                for t in range(len(j_3) - 1):
                    self.phe_data[j_3[t]][j_3[t + 1]] += self.ant_gener_phe / way_length_list[k]
            # Step 4: 当代的首尾总总结工作
...
class GA(baseAlgorithm):
    ...
    def __init__(self, map_data, chromes_list, population=50, max_iter=100, cross_pro=0.95, mut_pro=0.15):
        """
            Function
            --------
            初始化一个基本GA用于解决路径规划。
        ...
        self.chromes_list = chromes_list  # 初代染色体信息库, 初始化由外部提供
    ...
    def run(self, show_process=False):
        ...
        for i in range(self.max_iter):
            ...
            self.mutation()
            # Step 4: 子代并入父代中
            self.chromes_list = self.child_list.copy()
            if show_process:
                print('第', i, '代:')
        ...
        """
            ...
            -------
                child_1:    list-list
                    子代的染色体信息
        """
        chromes_1 = self.chromes_list.copy()
        child_1 = []
        for i in range(self.population):
            a_1 = []  # 3个选手
            b_1 = []  # 3个选手的成绩
            for j in range(3):
                a_1.append(np.random.randint(0, len(chromes_1)))
            for j in a_1:
                b_1.append(self.eval_fun(chromes_1[j]))
            c_1 = b_1.index(min(b_1))  # 最好的是第几个
            child_1.append(chromes_1[a_1[c_1]])  # 最好者进入下一代
        return child_1
        ...
        for i in range(0, len(child_1)):
            ...
            if dot_product < 0:  # 如果两个向量之间的夹角为锐角，则必产生冗余
                ...
                if 0 in v1:  # 如果第一个向量是水平或者垂直的,则必存在冗余
                    del child_1[i][index]  # 删除被选中的节点
                elif self.map_data[(point_a[0] + point_c[0]) // 2, (point_a[1] + point_c[
                    1]) // 2] == 0:  # 如果第一个向量是对角的,且point_a与point_b之间的节点为可通行，则可优化
                    child_1[i][index] = [(point_a[0] + point_c[0]) // 2,
                                         (point_a[1] + point_c[1]) // 2]  # 修改被选中的节点为中间节点
