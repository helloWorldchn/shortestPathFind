...
class GridFig:
    def __init__(self, map_data, x_label='', y_label='', startPoints=[], endPoints=[]) -> None:
        ...
        for ki, i in enumerate(map_data):
            for kj, j in enumerate(i):
                ...
                elif [kj, ki] in startPoints:
                    rect_pat.append(pat.Rectangle((kj, ki), 1, 1, color='blue'))
                elif [kj, ki] in endPoints:
                    rect_pat.append(pat.Rectangle((kj, ki), 1, 1, color='red'))
                else:
    ...
    def draw_way(self, way_data, lineStyle=None, color=None, marker=None, label=None):
        ...
        self.ax1.plot(way_data[:, 1] + 0.5, way_data[:, 0] + 0.5, linestyle=lineStyle, color=color, marker=marker,
                      label=label)
    ...
    def __init__(self, data_list, style_list, legend_list, xLabel='x', yLabel='y') -> None:
        ...
        for i in range(len(data_list)):
            if style_list[i][0] != '#':
                ...
                self.ax.plot(range(len(data_list[i])), data_list[i], color=style_list[i][:7],
                             marker=style_list[i][7] if len(style_list[i]) > 7 else ',',
                             linestyle=style_list[i][8:] if len(style_list[i]) > 8 else '-', markersize=4)
